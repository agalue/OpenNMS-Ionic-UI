...
export class RpnEvaluator {
  ...
  private buildOperators() {
    ...
    var funcOp = function (op, numArgs) {
      return function (stack, context) {
        ...
        for (var i = 0; i < numArgs; i++) {
          values[i] = self.valueOf(stack.pop(), context);
        }
        values.reverse();
    ...
    var limitOp = function (stack, context) {
      var max = self.valueOf(stack.pop(), context);
      var min = self.valueOf(stack.pop(), context);
      var val = self.valueOf(stack.pop(), context);
    ...
    var addNanOp = function (stack, context) {
      var b = self.valueOf(stack.pop(), context);
      var a = self.valueOf(stack.pop(), context);
      if (isNaN(a) && isNaN(b)) {
        return NaN;
      } else if (isNaN(a)) {
        return b;
    ...
    this.operators['ATAN2'] = funcOp(function(a,b) { return Math.atan2(a,b); }, 2);
  ...
  evaluate(rpn, context) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (this.isOperator(token)) {
        stack.push(this.eval(token, stack, context));
      } else {
        stack.push(token);
      }
    }
